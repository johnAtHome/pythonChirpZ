...
if __name__ == '__main__':
    ...
    freq1 = cztFreq(mFFT,f1,f2)
    z1 = czt(h1,mFFT,f1,f2,fs)
    z2 = czt(h2,mFFT,f1,f2,fs)
    
    
    nFFT=1024*8
    #freq0 =np.arange(len(t1))/len(t1)*fs - fs/2
    ...
    print (y2.shape)
    #plt.plot(freq0,y2[:,0])
    #plt.plot(freq0,np.abs(y1))

    fig,ax = plt.subplots(2,1)
    
    ax[0].plot(freq1,np.abs(z1[:,0]),label="czt")
    ax[0].plot(freq0,np.abs(y1[:,0]),label="FFT")
    
    ax[1].plot(freq1,np.abs(z2[:,0]),label="czt 0")
    ax[1].plot(freq1,np.abs(z2[:,1]),label="czt 1")
    ax[1].plot(freq1,np.abs(z2[:,2]),label="czt 2")
